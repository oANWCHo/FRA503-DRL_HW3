...
class MC_REINFORCE_network(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x - x.max(dim=-1, keepdim=True)[0]  # Stabilize softmax
...
class MC_REINFORCE(BaseAlgorithm):
    ...
    def generate_trajectory(self, env):
        ...
        while not done:
            # probs = self.policy_net(state)  # [1, num_actions] 1
            probs = self.policy_net(state).squeeze(0)

            if torch.isnan(probs).any() or torch.isinf(probs).any() or probs.sum().item() == 0:
                print("[Warning] Invalid probs detected, using uniform fallback.")
                probs = torch.ones_like(probs) / probs.size(0)  # shape [num_actions]

            dist = torch.distributions.Categorical(probs)
            action = dist.sample()
            log_prob = dist.log_prob(action)  # shape: [1]

            # action_tensor = torch.tensor([[action.item()]], dtype=torch.int64) 2
            ...
            # state = self._unwrap_obs(next_state)
            # state = torch.tensor(state, dtype=torch.float32, device=self.device).unsqueeze(0)
            # state = torch.tensor(next_state["policy"][0], dtype=torch.float32, device=self.device).unsqueeze(0) 
            state = next_state
            timestep += 1

        # Stack into tensors
        log_probs = torch.stack(log_prob_actions).to(self.device).requires_grad_()           # shape: [T]
        ...
        return ep_return, returns, log_prob_actions, trajectory
    ...
    def update_policy(self, returns, log_prob_actions):
        ...
        loss = self.calculate_loss(returns, log_prob_actions)
